...
export default function TopNav({ circuits, circuitId, onCircuitChange, user, isGuest, onLogout }: Props) {
  return (
    <nav className="fixed top-0 left-0 z-50 flex h-16 w-full items-center justify-between border-b border-outline-variant/25 bg-obsidian/85 px-4 backdrop-blur-xl md:px-6">
      <div className="flex min-w-0 items-center gap-4">
        ...
        {circuits.length > 0 && (
          <div className="hidden items-center gap-1 border-l border-outline-variant/30 pl-4 md:flex">
            <span className="mr-1 font-mono-data text-[10px] uppercase tracking-[0.25em] text-outline">
              Circuit
            </span>
            <div className="flex flex-wrap gap-1">
              {circuits.map((c) => (
                <button
                  key={c.circuit_id}
                  type="button"
                  onClick={() => onCircuitChange(c.circuit_id)}
                  className={`rounded border px-2.5 py-1 font-mono-data text-[11px] font-semibold uppercase tracking-wider transition-colors ${
